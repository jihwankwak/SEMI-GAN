...
log_name = 'date_{}_data_{}_model_{}_{}_lr_{}_{}_{}_gan_hidden_dim_{}_batch_size_{}_noise_d_{}_sample_num_{}'.format(
    args.date,
    args.dataset,
...
mean_model_spec = 'data_{}_batch_{}_model_{}'.format(args.dataset, args.batch_size, args.mean_model_type)

# gan model architecture ( naming for training & sampling )
gan_model_spec = 'data_{}_batch_{}_model_{}_noise_d_{}_hidden_dim_{}_'.format(args.dataset, args.batch_size, args.gan_model_type, args.noise_d, args.gan_hidden_dim)
...
mean_train_iterator = torch.utils.data.DataLoader(mean_train_dataset_loader, batch_size=args.batch_size, shuffle=True)

mean_val_iterator = torch.utils.data.DataLoader(mean_val_dataset_loader, batch_size=args.batch_size, shuffle=False)

mean_test_iterator = torch.utils.data.DataLoader(mean_test_dataset_loader, batch_size=1, shuffle=False)
...
if args.mode == 'train' and not os.path.isfile('./mean_models/'+mean_model_spec):
    ...
    torch.save(mean_best_model.state_dict(), './mean_models/'+mean_model_spec)
else :
    print('Load mean model----------------')
    mean_mytrainer.model.load_state_dict(torch.load('./mean_models/'+mean_model_spec))
...
noise_train_iterator = torch.utils.data.DataLoader(noise_train_dataset_loader, batch_size=args.batch_size, shuffle=True)

noise_val_iterator = torch.utils.data.DataLoader(noise_val_dataset_loader, batch_size=args.batch_size, shuffle=False)

noise_test_iterator = torch.utils.data.DataLoader(noise_test_dataset_loader, batch_size=1, shuffle=False)
...
if args.mode == 'train' and not os.path.isfile('./gen_models/'+gan_model_spec):
    ...
    torch.save(generator.state_dict(), './gen_models/'+gan_model_spec)
    if not os.path.exists('./dis_models'):
        os.makedirs('./dis_models')
    torch.save(discriminator.state_dict(), './dis_models/'+gan_model_spec)
else : 
    gan_mytrainer.G.load_state_dict(torch.load('./gen_models/'+gan_model_spec))
...
print("mean_train mean, std for scaling: ", mean_train_mean, mean_train_std)    

# noise result
noise_train_mean = noise_train_dataset_loader.data_y_mean
noise_train_std = noise_train_dataset_loader.data_y_std

print("noise_train mean, std for scaling: ",noise_train_mean, noise_train_std)
...
if args.mode == 'eval':
    ...
    test_mean_iterator = torch.utils.data.DataLoader(test_mean_dataset_loader, batch_size=1, shuffle=False)
