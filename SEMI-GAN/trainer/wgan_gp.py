...
class GanTrainer(trainer.gan_GenericTrainer):
    def __init__(self, noise_trainer_iterator, noise_val_iterator, generator, discriminator, optimizer_g, optimizer_d, exp_gan_lr_scheduler, noise_d, gp_strength):
    ...
    def train(self):
        ...
        for i, data in enumerate(self.train_iterator):
            ...
            gradient_penalty = self.calculate_gradient_penalty(data_y.data, gen_y.data, data_x.data)
    ...
    def calculate_gradient_penalty(self, real_data, fake_data, conditional_x):
        batch_size = real_data.size(0)

        eta = torch.FloatTensor(batch_size, 1).uniform_(0,1)
        eta = eta.expand(batch_size, real_data.size(1))
...
#                 interpolated = interpolated
        ...
        prob_interpolated = self.D(interpolated, conditional_x)

        # calculate gradients of probabilities with respect to examples
        gradients = autograd.grad(outputs=prob_interpolated, inputs=interpolated,
                                   grad_outputs=torch.ones(
                                   prob_interpolated.size()).cuda(), #if self.cuda else torch.ones(prob_interpolated.size()),
                                   create_graph=True, retain_graph=True)[0]

        grad_penalty = self.gp_strength * ((gradients.norm(2, dim=1) - 1) ** 2).mean()
        return grad_penalty